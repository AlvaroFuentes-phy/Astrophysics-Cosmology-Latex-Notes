...
En este tema buscamos dar respuesta a la siguiente pregunta: 
\begin{center}
  \textbf{¿como podemos ubicar las estrellas en el cielo nocturno?} 
\end{center}
Las coordenadas astronómicas sirven para definir la posición de un cuerpo celeste en el cielo cuando lo observamos desde un punto en concreto. Como paso previo al desarrollo de estas coordenadas, vamos dar una serie de definiciones básicas. 
...
\begin{figure}
    \centering
    \includegraphics[width=0.95\linewidth]{esfera_celeste.png}
...
El centro de este sistema de coordenadas se sitúa donde está el observador, por lo tanto estas coordenadas dependen de la ubicación del observador. En este sistema encontramos dos variables; el \textbf{acimut} y la \textbf{altura} 
...
Kepler postulo sus tres leyes de forma observacional, con el objetivo de describir el movimiento de los astros alrededor del Sol, posteriormente Newton haciendo uso de sus recientemente desarrollado calculo diferencial las demostró matemáticamente, enunciemos estas tres leyes. 
...
Llegados a este punto nos es natural hacernos las siguiente pregunta,
\begin{center}
¿habrá una \textbf{definición} (y una ecuación) \textbf{común} para \textbf{todas} las cónicas?  
\end{center}
...
\]
\textbf{El movimiento se desarrolla en el plano normal a \(\vec L\).} Podemos hacer todo este desarrollo aplicando coordenadas polares:
\[
\vec L = \mu\, \vec r \times \vec v
...
\end{equation*}
Buscamos obtener una ecuación de segunda grado que quedara de la forma
\begin{equation}
    \label{eq_2}
...
Esta ley nos dice que la intensidad máxima de emisión para un cuerpo negro depende de la temperatura del mismo. Para un cuerpo negro a una temperatura T, podemos usar la siguiente expresión para calcular su $\lambda_{max}$ 
...
si despejamos; 
\begin{equation*}
    \frac{2 h c e^{\frac{hc}{kT\lambda}}}{kT\lambda (e^{\frac{hc}{kT\lambda}} - 1)} = 10 \rightarrow \lambda_{\text{max}} T =
...
Por lo tanto, podemos reescribir las expresiones del efecto Doppler clásico usando este factor para el emisor 
\begin{align*}
...
El cambio de temperatura de una capa esférica a la
siguiente dependerá del gradiente de presiones
en dicha capa, que viene dado por la condición de equilibrio hidrostático. 
\begin{equation*}
...
\\
La expresión aquí encontrada permite interpretar que el cambio en la energía cinética media de una partícula del gas se debe al cambio en su energía potencial: 
\begin{equation*}
...
\end{equation*}
\end{formula}
Cuya solución usando la función $P(r)$ dada por la solución de la ecuación de equilibrio hidrostático nos dara le perfil radial de temperaturas $T(r)$, que ha de cumplir la condiciones de contorno: 
\begin{align*}
    T(r=0)=T_0
    \\
    T(r=R)=T_{eff} \approx0
...
Las soluciones de las ecuaciones diferenciales son, \textbf{para la masa:}
\begin{formula}
   \begin{equation}
...
para la ecuación del \textbf{balance de energía:} 
\begin{formula}
\begin{equation}
    L=\frac{\pi}{4}\varepsilon\rho_0 R^3
\end{equation}    
\end{formula}
y de la \textbf{ecuación de transporte radiativo} obtenemos: 
\begin{formula}
\begin{equation}
...
la primera conclusión que podemos obtener es una expresión particular para la  \textbf{ecuación de estado:} 
\begin{equation}
