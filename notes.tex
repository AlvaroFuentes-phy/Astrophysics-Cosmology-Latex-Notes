...
\begin{equation}
    ...
    \label{flujo_total}
\end{equation}
Al resolver \ref{flujo_total} se obtiene
\begin{equation*}
    F=\frac{2\pi h}{c^2}\int_0^\infty \frac{\nu^3}{\exp\!\left(\frac{h\nu}{k_B T}\right)-1}\, d\nu
    =\frac{2\pi k_B^4}{c^2h^3}T^4\int_0^\infty \frac{x^3}{e^x-1}\, dx
    = \sigma T^4,
\end{equation*}

usando el cambio de variable $x=\frac{h\nu}{k_B T}$ y la identidad
\begin{equation*}
...
\begin{equation*}
\lambda_{\max}T=
\frac{h c\, e^{\frac{hc}{k_B T\lambda}}}{5k_B\left(e^{\frac{hc}{k_B T\lambda}} - 1\right)}.
\end{equation*}
Para $\frac{hc}{k_B T\lambda}\gg 1$ se puede aproximar $e^{\frac{hc}{k_B T\lambda}}-1\simeq e^{\frac{hc}{k_B T\lambda}}$, y finalmente
\begin{equation*}
\lambda_{\max}T\simeq\frac{hc}{5k_B}.
\end{equation*}
\end{proof}

Nos interesa aplicar una correción sobre la temperatura del cuerpo negro que vendrá dada por
\begin{tcolorbox}
