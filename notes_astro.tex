...
\begin{flushright}
    ...
    porque fuiste la primera persona \\ en preguntarme acerca de las estrellas, \\ y ahora sé que tengo una, pequeñita pero firme.
    \\
    Gracias por tanto, abuela.}
\end{flushright}
\clearpage
A cada tema de estos apuntes le precede una fotografía como la que encontramos a continuación; son imágenes tomadas del proyecto \textbf{Astronomy Picture of the Day} de la NASA. Con la finalidad de mostrar algunos de los objetos que podemos encontrar en el Cosmos de una forma divulgativa y amena. A lo largo del contenido de estos apuntes, se incluyen pequeños ejemplos tomados como ejercicios de clase con su solución como una herramienta para ver aplicaciones directas de la teoría que estamos tratando. 
...
\chapter{Astronomía de posición. Coordenadas astronómicas}
\begin{flushleft}
...
En este tema buscamos dar respuesta a la siguiente pregunta: \textbf{¿cómo podemos ubicar las estrellas en el cielo nocturno?} Las coordenadas astronómicas sirven para definir la posición de un cuerpo celeste en el cielo cuando lo observamos desde un punto en concreto. 

Como paso previo al desarrollo de estas coordenadas, vamos a dar una serie de definiciones básicas. 
...
Es la línea imaginaria que une al punto que se encuentra justo por encima del observador (Cenit) con su punto opuesto (Nadir).
\end{definicion}
...
Estas definiciones nos ayudarán posteriormente para establecer sistemas de coordenadas que nos sirvan para localizar los objetos celestes que estudiamos.
...
Es el ángulo medido desde el horizonte astronómico hasta el centro del astro. Su valor es positivo cuando el astro es visible, siendo el máximo $90^\circ$ (cenit) y negativo cuando se encuentra por debajo del horizonte. 
\end{definicion}

\begin{definicion}[Azimut]
Es el ángulo medido desde el sur girando en sentido horario hasta la vertical del astro, es decir, el ángulo que forma el meridiano con el círculo vertical que pasa por un punto en la esfera celeste. 
\end{definicion}
...
La culminación de un astro ocurre cuando este pasa por el meridiano del lugar (es el punto más alto en su movimiento).
\end{definicion}
...
Es el ángulo que forma la visual del astro con el plano ecuatorial. Es una coordenada universal, válida desde cualquier punto de la Tierra. Comienza en $0^\circ$ sobre el ecuador y alcanza $90^\circ$ en el polo norte celeste y $-90^\circ$ en el polo sur celeste.
\end{definicion}
...
Repasemos con detalle las ecuaciones del problema de los dos cuerpos. Trabajaremos con las coordenadas generalizadas $r$ y $\phi$.

\subsection{Ecuaciones de movimiento para $r$ y $\phi$}

Partimos de la ecuación de movimiento para una fuerza central gravitatoria:
\begin{formula}
\begin{equation}
\mu\,\ddot{\vec r}=-\frac{K}{r^{2}}\,\vec e_r.
\end{equation}
\end{formula}

En coordenadas polares ($\vec r=r\,\vec e_r$) se tiene
\begin{align*}
\dot{\vec r} &= \dot r\,\vec e_r + r\dot\phi\,\vec e_\phi, \\
\ddot{\vec r} &= (\ddot r-r\dot\phi^{2})\,\vec e_r + (r\ddot\phi + 2\dot r\dot\phi)\,\vec e_\phi.
\end{align*}

Sustituyendo en $\mu\ddot{\vec r}=-\frac{K}{r^2}\vec e_r$ y separando componentes:
\begin{formula}
\begin{equation}
\begin{cases}
\mu(\ddot r-r\dot\phi^{2})=-\dfrac{K}{r^{2}}, \\
\mu(r\ddot\phi + 2\dot r\dot\phi)=0.
\end{cases}
\end{equation}
\end{formula}

La ecuación para $\phi$ equivale a la conservación del momento angular. En efecto,
\[
\vec L=\mu\,\vec r\times\dot{\vec r}=\mu r^{2}\dot\phi\,\vec e_z\equiv \ell\,\vec e_z,
\]
por lo que
\begin{formula}
\begin{equation}
...
\subsubsection*{Cambio de variable $t\to\phi$}

Usando la regla de la cadena,
\[
\dot r=\frac{dr}{dt}=\frac{dr}{d\phi}\,\dot\phi
=\frac{\ell}{\mu r^{2}}\,\frac{dr}{d\phi}.
\]

Derivando de nuevo y volviendo a aplicar la regla de la cadena,
\[
\ddot r
=\frac{d}{dt}(\dot r)
=\dot\phi\,\frac{d}{d\phi}(\dot r)
=\frac{\ell}{\mu r^{2}}\,\frac{d}{d\phi}\left(\frac{\ell}{\mu r^{2}}\,\frac{dr}{d\phi}\right)
=\frac{\ell^{2}}{\mu^{2}r^{2}}\,\frac{d}{d\phi}\left(\frac{1}{r^{2}}\,\frac{dr}{d\phi}\right).
\]

Introducimos el cambio habitual
\begin{formula}
\begin{equation}
 u\equiv \frac{1}{r}.
\end{equation}
...
Entonces
\[
\frac{dr}{d\phi}=\frac{d}{d\phi}\left(\frac{1}{u}\right)=-\frac{1}{u^{2}}\,\frac{du}{d\phi}=-r^{2}\frac{du}{d\phi},
\qquad
\frac{1}{r^{2}}\frac{dr}{d\phi}=-\frac{du}{d\phi},
\]
lo que lleva a
\begin{formula}
\begin{equation}
\ddot r=-\frac{\ell^{2}}{\mu^{2}r^{2}}\,\frac{d^{2}u}{d\phi^{2}}.
\end{equation}
\end{formula}

Sustituyendo en la ecuación radial $\mu\ddot r=\frac{\ell^{2}}{\mu r^{3}}-\frac{K}{r^{2}}$ y usando $u=1/r$:
\[
\mu\left(-\frac{\ell^{2}}{\mu^{2}r^{2}}\frac{d^{2}u}{d\phi^{2}}\right)=\frac{\ell^{2}}{\mu r^{3}}-\frac{K}{r^{2}}
\quad\Rightarrow\quad
\frac{d^{2}u}{d\phi^{2}}+u-\frac{\mu K}{\ell^{2}}=0.
\]

Definimos $y\equiv u=1/r$ y, para eliminar el término constante, $z\equiv y-\frac{\mu K}{\ell^{2}}$. Así se obtiene
\[
\frac{d^{2}z}{d\phi^{2}}+z=0.
\]

La ecuación homogénea tiene solución armónica. Por tanto,
\begin{formula}
\begin{equation}
...
deshaciendo los cambios $z=y-\dfrac{\mu K}{\ell^2}$ e $y=u=\dfrac{1}{r}$:
\[
\frac{1}{r}=u(\phi)=\frac{\mu K}{\ell^2}+A\cos(\phi-\phi_0).
\]

Tomando la inversa y reorganizando, resulta
\[
 r(\phi)=\frac{\ell^2/(\mu K)}{1+\left(\dfrac{A\ell^2}{\mu K}\right)\cos(\phi-\phi_0)}.
\]

Definimos dos parámetros orbitales (constantes):
\begin{formula}
\begin{equation}
\alpha\equiv \frac{\ell^2}{\mu K},
\label{semi_latus}
\end{equation}
\end{formula}
\begin{formula}
\begin{equation}
\varepsilon\equiv \frac{A\ell^2}{\mu K},
\label{excen}
\end{equation}
\end{formula}

\textbf{y obtenemos la ecuación polar de la cónica:}
\begin{formula}
\begin{equation}
 r(\phi)=\frac{\alpha}{1+\varepsilon\cos(\phi-\phi_0)}.
\end{equation}
\end{formula}

Si elegimos el origen angular en el perihelio (radio mínimo), entonces $\phi_0=0$ y queda
\begin{equation}
r(\phi)=\frac{\alpha}{1+\varepsilon\cos\phi}.
\end{equation}


Veamos un ejemplo breve (muy útil en clase) que conecta energía y semieje mayor.

\begin{ejemplo}
Demuestre que la energía de una órbita depende únicamente del semieje mayor $a$, usando los puntos de retorno.

Partimos de la energía mecánica escrita con el potencial efectivo:
\begin{equation}
E=T+U_{\mathrm{ef}}(r),
\qquad
U_{\mathrm{ef}}(r)=\frac{\ell^2}{2\mu r^2}-\frac{K}{r}.
\end{equation}

En los puntos de retorno (periapsis y apoapsis) se cumple $\dot r=0$ y, por tanto, $T=\tfrac12\mu\dot r^{2}=0$. Luego
\[
E=U_{\mathrm{ef}}(r)=\frac{\ell^2}{2\mu r^2}-\frac{K}{r}.
\]
Multiplicando por $2\mu r^2$ se obtiene una cuadrática en $r$:
\begin{equation}
2\mu E\,r^2+2\mu K\,r-\ell^2=0.
\label{eq_2}
\end{equation}

Sus dos soluciones son $r_{\min}$ y $r_{\max}$:
\[
 r_{\min,\max}=-\frac{K}{2E}\left(1\mp\sqrt{1+\frac{2E\ell^2}{\mu K^2}}\right),
\qquad (E<0).
\]

El semieje mayor se define como $a=\tfrac12(r_{\min}+r_{\max})$, luego
\[
 a=-\frac{K}{2E}
\quad\Rightarrow\quad
E=-\frac{K}{2a}.
...
Un planeta completa una órbita elíptica en un tiempo $T$ (periodo).
Sea $A$ el área de la elipse. Geométricamente, el área de la elipse vale:
\begin{equation}
A = \pi a b = \pi a^2 \sqrt{1-\varepsilon^2}.
\end{equation}
Donde $a$ es el semieje mayor, $b$ es el semieje menor, y $\varepsilon$ es la excentricidad, siendo la relación entre los semiejes:
\begin{equation}
b = a\sqrt{1-\varepsilon^2}.
\end{equation}
Por la conservación de la \textbf{velocidad areolar} el área de la elipse también se relaciona con el periodo $T$ y el momento angular $L$:
\begin{equation}
A = T\,\frac{dA}{dt} = \frac{T L}{2\mu}.
\end{equation}

Igualando ambas expresiones para el área $A$:
\begin{equation}
\pi a^2 \sqrt{1-\varepsilon^2} = \frac{T L}{2\mu}.
\end{equation}

Elevando al cuadrado esta relación, obtenemos:
\begin{formula}
    \begin{equation}
...
Para la elipse, el semieje mayor $a$ se relaciona con el parámetro $\alpha$ y la excentricidad $\varepsilon$ mediante la siguiente relación geométrica:
\begin{equation}
a = \frac{\alpha}{1-\varepsilon^2}.
\end{equation}

Siendo $\alpha$ el parámetro dinámico definido como:
\begin{equation}
\alpha = \frac{L^2}{\mu K}.
\end{equation}

De donde resulta el semieje mayor $a$ al sustituir $\alpha$:
\begin{equation}
a = \frac{L^2}{\mu K (1-\varepsilon^2)}.
\end{equation}

Reordenando esta expresión para despejar el término $\frac{L^2}{\mu}$, se obtiene,
\begin{formula}
  \begin{equation}
...
Sustituimos \ref{ecuacion2} en \ref{ecuacion1}:
\begin{equation*}
  \pi^2 a^4 (1-\varepsilon^2) = \frac{T^2}{4 \mu^2} \left[ \mu K a (1-\varepsilon^2) \right].
\end{equation*}


Simplificando los términos $a$, $\mu$, y $(1-\varepsilon^2)$ en ambos lados de la ecuación y reordenando, se obtiene:
\begin{equation}
\frac{a^3}{T^2} = \frac{K}{4 \pi^2 \mu}.
\end{equation}


Sustituyendo las definiciones de la masa reducida ($\mu$) y la constante ($K$):
\begin{equation}
...
\begin{figure}
    \centering
    \includegraphics[width=1\linewidth]{M78.png}
    \caption{En el vasto complejo de la Nube Molecular de Orión, destacan varias nebulosas de un azul brillante. En el centro de esta imagen se encuentran dos de las nebulosas de reflexión más prominentes: nubes de polvo iluminadas por la luz reflejada de estrellas brillantes incrustadas en su interior. La más famosa es M78, situada en el centro de la imagen y catalogada hace más de 200 años. A su izquierda, en la parte superior, se encuentra la menos conocida NGC 2071.
    \\
    Los astrónomos continúan estudiando estas nebulosas de reflexión para comprender mejor cómo se forman las estrellas en su interior. El resplandor rojo general proviene del gas de hidrógeno difuso que cubre gran parte del complejo de Orión, el cual se extiende por casi toda la constelación de Orión. Cerca de este gran complejo, que se encuentra a unos 1,500 años luz de distancia, se hallan la Nebulosa de Orión, la Nebulosa Cabeza de Caballo y el Bucle de Barnard (que se aprecia parcialmente aquí como la banda blanca en la parte superior izquierda).}
    \label{fig:placeholder}
\end{figure}
...
La información que recibimos del cosmos es, en su mayor parte, radiativa. No obstante, desde la primera detección de ondas gravitacionales en 2015, este nuevo mensajero ha adquirido un papel creciente en la investigación astrofísica. En este tema nos centraremos en los aspectos fundamentales de la radiación electromagnética procedente de las estrellas.

Las radiaciones electromagnéticas describen procesos de emisión y propagación de energía, ya sea en forma ondulatoria o corpuscular, y pueden propagarse tanto en un medio material como en el vacío. Se distinguen entre sí por su frecuencia: a mayor frecuencia, mayor energía.

Las radiaciones electromagnéticas se organizan en el espectro electromagnético (tabla \ref{tab:espectroEM2}).
...
Las radiaciones de alta frecuencia (superiores a $10^{17}\,\text{Hz}$) son ionizantes y, al interaccionar con la materia, pueden producir la ionización de sus átomos; por ello, este tipo de radiación supone un riesgo.

\section{Radiación del cuerpo negro}
\begin{tcolorbox}
Se denomina \textbf{cuerpo negro} al sistema ideal que absorbe toda la radiación incidente. La radiación que emite depende únicamente de su temperatura y de la frecuencia.
\end{tcolorbox}

Partiendo de la expresión del flujo específico $F_{\nu}$:
\begin{equation}
    F_{\nu}=\frac{2\pi h}{c^2}\,\frac{\nu^3}{\exp\!\left(\frac{h\nu}{k_B T}\right)-1}
    \label{Flujo_e}
\end{equation}

Podemos definir el flujo total integrando sobre todo el espectro de frecuencias:
\begin{equation}
    F=\int_0^\infty F_{\nu}\, d\nu.
    \label{flujo_total}
\end{equation}
Al resolver \\ref{flujo_total} obtenemos, sustituyendo \\ref{Flujo_e}:
\\begin{equation*}
    F
    =\\int_0^\\infty F_{\\nu}\\,d\\nu
    =\\frac{2\\pi h}{c^2}\\int_0^\\infty \\frac{\\nu^3}{\\exp\\!\\left(\\frac{h\\nu}{k_B T}\\right)-1}\\, d\\nu.
\\end{equation*}
Haciendo el cambio de variable $x=\\dfrac{h\\nu}{k_B T}$ (esto es, $\\nu=\\dfrac{k_B T}{h}x$ y $d\\nu=\\dfrac{k_B T}{h}dx$), queda
\\begin{equation*}
    F=\\frac{2\\pi k_B^4}{c^2h^3}T^4\\int_0^\\infty \\frac{x^3}{e^x-1}\\, dx.
\\end{equation*}
Usando la identidad
\\begin{equation*}
    \\int_0^\\infty \\frac{x^3}{e^x-1}\\,dx=\\frac{\\pi^4}{15},
\\end{equation*}
se llega a $F=\\sigma T^4$.
...
El parámetro $\sigma$ es la constante de Stefan--Boltzmann, definida por
\begin{equation}
\sigma=\frac{2\pi^5k_B^4}{15c^2h^3}
= 5.6693 \times 10^{-8}\,\si{\watt\per\meter\squared\per\kelvin\tothe{4}}.
    \label{cosntS_B}
\end{equation}

Hemos obtenido una relación directa entre el flujo emitido por un cuerpo negro y su temperatura. Esta expresión será de gran utilidad, pues en primera aproximación supondremos que las estrellas se comportan como cuerpos negros.

\begin{figure} [h]
    \centering
    \includegraphics[width=1\linewidth]{cuerpo_negrog.png}
    \caption{En la figura observamos cómo varía la intensidad de la emisión en función de la longitud de onda. La línea azul une los distintos máximos de emisión, que se desplazan a longitudes de onda menores a medida que aumenta la temperatura, tal y como describe la ley de desplazamiento de Wien.}
    \label{cuerpo_negro}
\end{figure}

\section{Ley de desplazamiento de Wien}
Esta ley establece que la longitud de onda correspondiente a la máxima emisión de un cuerpo negro depende de su temperatura. Para un cuerpo negro a temperatura $T$:
...
\\begin{proof}
Partiendo de \\ref{Flujo_e}, el máximo espectral se obtiene imponiendo
\\begin{equation*}
    \\frac{dF_{\\lambda}}{d\\lambda}=0.
\\end{equation*}
Un cálculo directo conduce a
\\begin{equation*}
\\frac{\\pi h c^{2}}{\\lambda^{6}\\left(e^{\\frac{hc}{k_B T\\lambda}} - 1\\right)}
\\left[
\\frac{2 h c\\, e^{\\frac{hc}{k_B T\\lambda}}}{k_B T\\lambda\\left(e^{\\frac{hc}{k_B T\\lambda}} - 1\\right)} - 10
\\right]=0.
\\end{equation*}
Como el prefactor no se anula, el máximo viene dado por
\\begin{equation*}
\\frac{2 h c\\, e^{\\frac{hc}{k_B T\\lambda}}}{k_B T\\lambda\\left(e^{\\frac{hc}{k_B T\\lambda}} - 1\\right)}=10.
\\end{equation*}
Reordenando,
\\begin{equation*}
\\lambda_{\\max}T=
\\frac{h c}{5k_B}\\,\\frac{e^{\\frac{hc}{k_B T\\lambda}}}{e^{\\frac{hc}{k_B T\\lambda}} - 1}.
\\end{equation*}
En el régimen de Wien, $\\dfrac{hc}{k_B T\\lambda}\\gg 1$, se cumple $e^{\\frac{hc}{k_B T\\lambda}}-1\\simeq e^{\\frac{hc}{k_B T\\lambda}}$, y por tanto
\\begin{equation*}
    \\lambda_{\\max}T\\simeq\\frac{hc}{5k_B}.
\\end{equation*}
\\end{proof}

Nos interesa aplicar una corrección sobre la temperatura del cuerpo negro que vendrá dada por
\begin{tcolorbox}
...
\begin{formula}
    ...
    \end{equation}
Donde, para llegar a la expresión anterior, hemos supuesto que la estrella se comporta como un cuerpo negro.
\end{formula}

\begin{definicion}[Luminosidad aparente o brillo]
La luminosidad aparente (o brillo) se define como el flujo de energía recibido en la Tierra a una distancia $d$ de la estrella.
\end{definicion}

Si no hay pérdida de energía, el flujo atraviesa superficies esféricas y se cumple
\begin{equation*}
...
Esta relación se obtiene bajo la hipótesis de emisión tipo cuerpo negro. Sin embargo, las estrellas no son cuerpos negros perfectos; por ello se introduce una temperatura equivalente.
...
Las variables intensivas dentro de una estrella
(presión, densidad, y temperatura) están relacionadas entre sí como un gas ideal. 
\\
Esto nos dice que hay equilibrio termodinámico
local: se puede definir una presión, una densidad, y una temperatura, pero sus
valores no son uniformes en toda la estrella, sino que su valor es distinto en cada capa esférica.
\\
La expresión
\begin{formula}
    \begin{equation*}
...
El cambio de temperatura de una capa esférica a la siguiente dependerá del
gradiente de presiones en dicha capa, que viene
dado por la condición de equilibrio hidrostático. 
\begin{equation*}
...
\begin{enumerate}[label=(\roman*)]
    ...
    \end{formula}
\end{enumerate}
Usando estas relaciones podemos conocer el \textbf{tiempo de vida} (nuclear) de una estrella en relación con su masa, tenemos que: 
\begin{equation*}
...
\begin{definicion}[Límite de Tolman--Oppenheimer--Volkoff]
El límite de Tolman--Oppenheimer--Volkoff (TOV) es un límite superior para la masa de estrellas compuestas de materia neutrónica degenerada (estrellas de neutrones). Cuando una estrella de neutrones lo supera, no puede mantenerse a sí misma y colapsa en un agujero negro.
\begin{formula}
    \begin{equation}
        M_{TOV}=2,55M_{\odot}
    \end{equation}
